...
conexao.query("Select * from usuarios where nomeusuario=? and senha=?".[req.body.nomeusuario,req.body.senha],
(error, data)=>{
...
app.put("/usuarios/atualizar/:id",(req,res)=>{
  ...
  [req.body,req.params.id],(error, data)=>{
    if (error) return res.status(500).send({output:`Erro ao tentar atualizar -> ${erro}`})
